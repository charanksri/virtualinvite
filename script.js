...
class Particle {
  ...
  update() {
    ...
    this.alpha =
      p < .1  ? p * 10 * this.maxAlpha :
      p > .85 ? (1 - p) * 6.67 * this.maxAlpha :
      this.maxAlpha;
...
class Bokeh {
  ...
  update() {
    ...
    const p = this.life / this.maxLife;
    this.alpha =
      p < .2  ? p * 5 * this.maxAlpha :
      p > .8  ? (1 - p) * 5 * this.maxAlpha :
      this.maxAlpha;
...
/* ──────────────────────────────────────────
   CSS FALLING PETALS
────────────────────────────────────────── */
(function spawnCSSPetals() {
  const colors = ['#f5b8a0','#f2a08e','#e8836a','#f7cdb8','#d4a07a','#c9893e'];
...
function openEnvelope() {
  ...
  // Inject fly-away keyframe dynamically
  const style = document.createElement('style');
  style.textContent = `
    @keyframes envFlyAway {
      0%   { opacity: 1; transform: scale(1) translateY(0); }
      100% { opacity: 0; transform: scale(.7) translateY(40px) rotate(3deg); }
    }
  `;
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        document.getElementById('invCard').classList.add('visible');
      }, 300);
    }, 80);
  }, 650);
}
